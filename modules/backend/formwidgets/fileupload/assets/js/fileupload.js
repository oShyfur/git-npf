...
+function ($) { "use strict";
    ...
    FileUpload.prototype.onUploadSuccess = function(file, response) {
        ...
        if (response.id) {
            //Multi image cropping generate unique preview
            var $previewImage = $(file.previewElement),
            $imgUniqueId = $('.image', $previewImage)
            $imgUniqueId.addClass('images_'+response.id);
    ...
    FileUpload.prototype.onUploadError = function(file, error) {
        ...
        if (extension == 'HEIC' || extension == 'heic'){
            var response = confirm("This type of image not supported. You can reformat this from another source . Want to proceed ?");
            if ( response == true ){window.open('https://heic.online', '_blank');}
        }else if(extension == 'jpg' || extension == 'png' || extension == 'jpeg' || extension == 'JPG' || extension == 'PNG' || extension == 'JPEG'){
            var response = confirm("Image size can not be grater then 4 MB. You can resize this from another source . Want to proceed ?");
            if ( response == true ){window.open('https://picresize.com', '_blank');}
        }else if(extension == 'pdf' || extension == 'PDF'){
            alert('File size can not be grater then 10 MB');
        }
        var $preview = $(file.previewElement)
    ...
    //         if ( response == true ){window.open('https://heic.online', '_blank');}
    //     }else if(extension == 'jpg' || extension == 'png' || extension == 'jpeg' || extension == 'JPG' || extension == 'PNG' || extension == 'JPEG'){
    //         var response = confirm("This image ratio is high. You can resize this from another source . Want to proceed ?");
    //         if ( response == true ){window.open('https://tinypng.com', '_blank');}
    //     }
    //     var $preview = $(file.previewElement)
    //     $preview.addClass('is-error')
    // }
