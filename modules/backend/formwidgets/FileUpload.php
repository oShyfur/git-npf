...
class FileUpload extends FormWidgetBase
{
    ...
    protected function prepareVars()
    {
        ...
        $this->vars['useCropping'] = $this->useCropping;
    ...
    public function onSaveAttachmentConfig()
    {
        try {
            ...
            if ($file = $formWidget->model) {
                ...
                $useCropping = $this->getConfig('useCropping');
                if ($displayMode == 'image-single' && $useCropping) {
                    $file = $file->cropImage(post());
                    return [
                        '.image' => '<img src="' . $file->path . '"/>',
                        'displayName' => $displayName
                    ];
                }elseif($displayMode == 'image-multi' && $useCropping){
                    $file = $file->cropImage(post());
                    return [
                        '.images_'.$file->id => '<img src="' . $file->path . '"/>',
                        'displayName' => $displayName
                    ];
                }
    ...
    public function onUpload()
    {
        try {
            ...
            // $megaByte = isset($this->maxFilesize)? $this->maxFilesize : 10;
            if($this->getAcceptedFileTypes() == 'pdf' || $this->getAcceptedFileTypes() == 'PDF'){
                $megaByte = isset($this->maxFilesize)? $this->maxFilesize : 12;
            }else{
                $megaByte = isset($this->maxFilesize)? $this->maxFilesize : 8;
            }
            $fileMaxSize = intval($megaByte)*1024;

            //   $validationRules = ['max:' . $fileModel::getMaxFilesize()];
            $validationRules = ['max:' . $fileMaxSize];
