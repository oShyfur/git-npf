...
class FormController extends ControllerBehavior
{
    ...
    /**
     * @var string The key to store Model Data in the cache under
     */
    public $cacheKey = 'backend::form.data';
    ...
    {
        $this->setfieldnameforValidation($model);
     ...
     * @return mixed
     */
    public function setfieldnameforValidation($model){
        $i = 0;
        $field = [];
        $fieldName = [];
        $translatables = $model->translatable;
        $rules = $model->rules;

        $a=json_encode((array)$model);
        $b=(array)json_decode(str_replace('\u0000*\u0000','',$a));
        $c = $b['validationDefaultAttrNames'];
        if(isset($rules)){
            foreach($rules as $r => $value){
                if(isset($translatables)){
                    foreach($translatables as $t => $val){
                        if ($r == $val) {
                            $field[$i] = $val;
                            $i = $i + 1;
                        }
            ...
            if(isset($field)){
                foreach ($field as $fl => $flValue) {
                    $fieldName[$flValue] = $c->$flValue;
                }

            }

        }
        Session::put('fieldName',$fieldName);
    }
    public function errorhandle($model){
        $i = 0;
        $field = [];
        $translatables = $model->translatable;
        $rules = $model->rules;

        $getFieldSSessions = Session::get('fieldName');
        if(isset($rules)){
            foreach($rules as $r => $value){
                if(isset($translatables)){
                    foreach($translatables as $t => $val){
            ...
            if(isset($field)){
                foreach ($field as $fl => $flValue) {
                    $sdata = post('RLTranslate');
                    if (isset($sdata)) {
                        foreach ($sdata as $locale => $_data) {
                            if (str_replace("\r\n",'', $_data[$flValue]) == '') {
                                return Flash::error('The '.$getFieldSSessions[$flValue].' '.$locale.' field is requried');
                            }
                        }
                    }
    ...
    public function create_onSave($context = null)
    {
        ...
        Db::transaction(function () use ($modelsToSave, $model) {
            foreach ($modelsToSave as $modelToSave) {
                $modelToSave->save(null, $this->formWidget->getSessionKey());
            }
            // np_structure_sites
            // if($model->table=='np_contents_news')
            // {
            //     FacadesDB::insert('insert into npf_domains (id, subdomain) values (?, ?)', [80090, 'muhammad.gov.bd']);
            // }
        });
        ...
        $this->cacheKey = 'backend::form.'.$model->className;
    ...
    /**
     * Clear Site Cache Data after the creation or updating form is saved.
     */
    private function deleteCacheData(){
        if(Session::has('site')) {
            $siteData =  Session::get('site');
            $siteId = $siteData['id'];
