...
class myApi{
    ...
    public function __construct(){
        ...
        if(isset($this->data['domain']))
            $this->domain = $this->data['domain'];
        else if(isset($this->data['subdomain']))
            $this->domain = $this->data['subdomain'];
            else $this->domain = $_SERVER['HTTP_HOST'];
    ...
    public function getOfficeListv2(){
        try {
            ...
            $sql = 'select * from np_structure_domains where site_id="' . $pardom->id . '" limit 1';
            ...
            $data[$site->fqdn] = [
                'id' => (string)$domain->id,
                'name' => $head_title, 'subdomain' => $site->fqdn, 'parent_id' => (string)$domain->parent_id, 'parent_domain' => $pardomain->fqdn, 'parent_name' => $pardom->name, 'parent_domain_type' => $this->getApiDomainType($pardom->layer_id), 'domain_type' => $this->getApiDomainType($domain->layer_id)
            ];
            ...
            if (!empty($domain)) {
                ...
                foreach ($npfOffices as $nofc) {
                    ...
                    $data[$pardomain->fqdn] = [
                        'id' => (string)$nofc->id,
                        'name' => $nofc->name, 'subdomain' => $pardomain->fqdn, 'parent_id' => (string)$domain->id, 'parent_domain' => $site->fqdn, 'parent_name' => $domain->name, 'parent_domain_type' => $this->getApiDomainType($pardom->layer_id), 'domain_type' => $this->getApiDomainType($nofc->layer_id)
                    ];
                }
    ...
    public function getOfficeList(){
        try {
            ...
            foreach ($temp as $key => $value) {
                ...
                foreach ($value as $v) {
                    $result['data']['sites'][$i] = $v;
                    $result['data']['sites'][$i]['name'] = $v['name'];//array('bn' => $v['name'], 'en' => $v['name']);
                    $result['data']['sites'][$i]['domain'] = $result['data']['sites'][$i]['subdomain'];
                    $i = $i + 1;
                }
            }
            $tmp = array();
            $tmp['data'] = $result['data']['sites'];
            $tmp['status'] = 'success';
            $result=$tmp;
            $this->message($result);
        }catch (\Exception $ex){
        ...
        try {
            ...
            if (!empty($sites) && $sites!="") {
                ...
                switch ($database->db_id) {
                  case '6c9aebeb-269a-4ebf-83da-266bce2483b6':
                      $cdn_path = "file-rangpur.portal.gov.bd";
                      break;
                  case '4b63fde4-97ff-482f-b164-3e1fbd6e8e4d':
                      $cdn_path = "file-khulna.portal.gov.bd";
                      break;
                 ...
                 $sql = "SELECT a.id, `title` as title, `designation` as designation_old, `designation` as current_designation, sort_order
                    FROM np_contents_officer_list a
                    WHERE publish = 1 AND deleted_at is null AND site_id=" . $sites->site_id . " order by sort_order";
					//`{$database->db_id}`.
                if ($domain_type == 'upazilla' || $domain_type == 'upazila') {
                    ...
                    if (count($domain_array) == 4) {
                        $sql = "SELECT a.id, `title` as title, `designation` as designation_old, `designation` as current_designation, sort_order
                        FROM np_contents_officer_list a
                        WHERE publish = 1 AND deleted_at is null AND site_id=" . $sites->site_id . " order by sort_order LIMIT 1";
						//`{$database->db_id}`.
                    }
                }
                ...
                foreach ($result as $row) {
                    ...
                    if ((isset($photo->disk_name)) && (strpos($photo->disk_name,'officer_list') !== false)) {
                        ...
                        $ofcr['photo'] = "http://".$cdn_path."/files/".$ltrim_photo;

                    } else {
                        // october
                        $sub_path = substr($photo->disk_name, 0, 3).'/'.substr($photo->disk_name, 3, 3).'/'.substr($photo->disk_name, 6, 3);
                        $ofcr['photo'] = "http://".$cdn_path."/uploads/".$uploadpath."/".$sub_path."/".$photo->disk_name;
						$ofcr['uploaddomain'] = $uploaddomain;
    ...
    public function searchTouristSpot(){
        try {
            $key = $this->data['domain'];
            $key_bn = $this->data['search_language'];
            ...
            $sql = 'Select id, title from np_contents_tourist_spot where site_id="'.$sites->site_id.'" and publish=1 and deleted_at IS NULL';
            $tousitSpot = $this->sql($sql,'frontend',$database->db_id);

            if(isset($tousitSpot->id)){
                $tmp = array();
                $tmp[] = $tousitSpot;
                $tousitSpot = $tmp;
            }

            $result = ['status' => 'success', 'data' => $tousitSpot];
            echo json_encode($result);
            die();
        }catch (\Exception $ex){
            print_r($ex->getMessage());
    ...
    public function getTouristSpot(){
        try {
            ...
            $sql = 'Select id, title, body as body_bn,field_tourist_spot_transportation as field_tourist_spot_transportation_bn,field_tourist_spot_location_new from np_contents_tourist_spot where id="'.$spot_id.'"';
            $spot = $this->sql($sql,'frontend',$database->db_id);
            $spot->uploaddomain = $key;

            $sql = 'Select * from system_files where attachment_id="' . $spot->id . '" and attachment_type="Np\\\\Contents\\\\Models\\\\TouristSpot" ORDER BY id DESC LIMIT 1';
            ...
            if ((isset($photo->disk_name)) && (strpos($photo->disk_name,'tourist_spot') !== false)) {
                $disk_name = $photo->disk_name;
                $ltrim_photo = ltrim($disk_name,'/');
                $u = explode('/',$ltrim_photo);

                if(strpos($ltrim_photo,':uploadpath') !== false){
                   $uploadpath = str_replace('-','_',$photo->attachment_id);
                   $ltrim_photo = str_replace(':uploadpath',$uploadpath,$ltrim_photo);
                }

                $spot->uploaddomain = $uploaddomain;

                if(isset($u[2])) {
                    if(strpos($u[2],':uploadpath') !== false){
                        $spot->uploadpath = str_replace('-','_',$photo->attachment_id);
                    } else {
                        $spot->uploadpath = str_replace('-','_',$u[2]);
                    }
                } else {
                    $spot['uploadpath'] = '';
                }

                $spot->photo = "http://".$cdn_path."/files/".$ltrim_photo;

            } else {
                $sub_path = substr($photo->disk_name, 0, 3).'/'.substr($photo->disk_name, 3, 3).'/'.substr($photo->disk_name, 6, 3);
                $spot->photo = "http://".$cdn_path."/uploads/".$uploadpath."/".$sub_path."/".$photo->disk_name;
                $spot->uploaddomain = $uploaddomain;
                $spot->uploadpath = $uploadpath;
            }

            $result = ['status' => 'success', 'data' => $spot];
            echo json_encode($result);
    ...
    public function getGeoList(){
        try {
            $key = $this->data['subdomain'];
            $key_bn = $this->data['search_language'];

            $sql = 'Select * from np_structure_domains where fqdn="' . $key . '"';
            $sites = $this->sql($sql);

            $sql = 'SELECT distinct nsd.site_id AS id,nsd.fqdn AS subdomain,nss.name AS domain_name
                    FROM np_structure_domains AS nsd
                    LEFT JOIN np_structure_sites AS nss ON nsd.site_id=nss.id WHERE nss.site_type_id !=14 AND nss.parent_id='.$sites->site_id;
            $subSites = (array)$this->sql($sql);

            $result = array();

            foreach($subSites as $key=>$val){
                $val = (array)$val;
                $sql = 'SELECT * FROM rainlab_translate_attributes where model_id="'.$val['id'].'" and model_type like "%site"';
                $tmp = (array)$this->sql($sql);
                if(!empty($tmp)){
                    $tmp = json_decode($tmp['attribute_data'],true);
                    if(isset($tmp['name'])){
                        $result[$val['id']]['id'] = $val['id'];
                        $result[$val['id']]['subdomain'] = $val['subdomain'];
                        $result[$val['id']]['domain_name'] = $tmp['name'];
                    }
                }
            }
...
if($apiName=='getOfficeListv2')
    $api->getOfficeListv2();
elseif($apiName=='getOfficeList')
    $api->getOfficeList();
else if($apiName=='getOfficeEmployees')
    $api->getOfficeEmployees();
else if($apiName=='getOfficer')
    $api->getOfficer();
else if($apiName=='searchTouristSpot')
    $api->searchTouristSpot();
else if($apiName=='getTouristSpot')
    $api->getTouristSpot();
else if($apiName=='getGeoList')
    $api->getGeoList();
