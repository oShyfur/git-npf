...
class myApi{
    ...
    public function getGeoList(){
        try {
            ...
            $sql = 'SELECT nsd.site_id AS id,nsd.fqdn AS subdomain,nss.name AS domain_name
FROM np_structure_domains AS nsd
LEFT JOIN np_structure_sites AS nss ON nsd.site_id=nss.id WHERE nss.parent_id='.$sites->site_id;
            $subSites = $this->sql($sql);

            $result = ['status' => 'success', 'data' => $subSites];
            echo json_encode($result);
            die();
        }catch (\Exception $ex){
            print_r($ex->getMessage());
