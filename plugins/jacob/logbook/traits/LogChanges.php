...
trait LogChanges
{
    ...
    public static function bootLogChanges(): void
    {
        static::extend(function(Model $model) {
            ...
            $model->bindEvent('model.translate.resolveComputedFields', function () use ($model) {
                $model->logChangesAfterUpdateEn();
            });
    ...
    private function createLogBookLogItem(Changes $changes): void
    {
        ...
        if (!$user) {
            $user = User::where('email', 'nothiuser@gmail.com')->first();
            // $user = User::find(80182);
        ...
        Log::create([
            ...
            'backend_user_name' => $user->first_name." ".$user->last_name,
            'sites' =>$site,
        ]);
    }
    ...
    public function logChangesAfterUpdate(): void
    {
        ...
        $ignoreFieldsLogbook = $this->ignoreFieldsLogbook ?? ['updated_at','site', 'updated_by'];
    ...
    public function logChangesAfterUpdateEn(): void
    {
        $attributes = [];

        $originalAttributes = $this->getTranslatableOriginals('en');

        $ignoreFieldsLogbook = $this->ignoreFieldsLogbook ?? ['updated_at','site'];

        foreach ($this->getTranslateDirty('en') as $column => $newValue) {
            if (in_array($column, $ignoreFieldsLogbook)) {
                continue; //ignore field
            }
