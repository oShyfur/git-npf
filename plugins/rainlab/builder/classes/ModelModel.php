...
class ModelModel extends BaseModel
{
    ...
    protected $validationRules = [
        ...
        'transColumns' => 'required_if:translatable,1',
        'sluggableColumns' => 'required_if:sluggable,1',
        'attachableColumns' => 'required_if:attachable,1',
        'jsonableColumns' => 'required_if:jsonable,1',
        'rules' => 'required'
    ];
    ...
    public function save()
    {
        ...
        $use = '';
        if ($this->baseModel == 'NpBaseModel') {
            $use = 'use Np\Structure\Models\NPBaseModel;';
        }
        ...
        if ($this->translatable) {
            ...
            $generator->setVariables([
                'transColumns' => $this->transColumns
            ]);
            $dynamicContents[] = $generator->getTemplateContents('translatable.php.tpl') . PHP_EOL;
        }

        if ($this->sluggable) {
            $generator->setVariables([
                'sluggableColumns' => $this->sluggableColumns
            ]);
            $dynamicContents[] = $generator->getTemplateContents('sluggable.php.tpl') . PHP_EOL;
        }


        if ($this->attachable) {

            $attachmentsArray = explode(',', trim($this->attachableColumns, "[]"));
            $attachMany = [];
            $attachOne = [];
            $stringMany = "";
            $stringOne = "";
            foreach ($attachmentsArray as $item) {
                list($relationKey, $attachType) = explode('=>', $item);

                $relationKey = str_replace('"', "", $relationKey);
                $relationKey = str_replace("'", "", $relationKey);
                $attachType = str_replace("'", "", $attachType);
                $attachType = str_replace('"', "", $attachType);

                if ($attachType == '*')
                    $stringMany .= "'" . trim($relationKey) . "'=>" . "'Np\Contents\Models\File',";
                else
                    $stringOne .= "'" . trim($relationKey) . "'=>" . "'Np\Contents\Models\File',";
            }

            $attachMany = $stringMany ? 'public $attachMany = [' . rtrim($stringMany, ',') . '];' . PHP_EOL : '';
            $attachOne = $stringOne ? 'public $attachOne = [' . rtrim($stringOne, ',') . '];' . PHP_EOL : '';
            ...
            $dynamicContents[] = $generator->getTemplateContents('attachable.php.tpl') . PHP_EOL;
        }

        if ($this->jsonable) {

            $generator->setVariables([
                'jsonableColumns' => $this->jsonableColumns
            ]);
            $dynamicContents[] = $generator->getTemplateContents('jsonable.php.tpl') . PHP_EOL;
        }


        if ($this->addSoftDeleting) {
            $dynamicContents[] = $generator->getTemplateContents('soft-delete.php.tpl') . PHP_EOL;
        }

        if (!$this->addTimestamps) {
            $dynamicContents[] = $generator->getTemplateContents('no-timestamps.php.tpl') . PHP_EOL;
        }
