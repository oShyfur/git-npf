...
abstract class TranslatableBehavior extends ExtensionBase
{
    ...
    public function __construct($model)
    {
        ...
        $host = input('domain');
        $this->model->bindEvent('model.beforeGetAttribute', function ($key) use ($model, $host) {
            if ($this->isTranslatable($key)) {
                $value =  $this->getAttributeTranslated($key);
                    //Cache::get($host.'-'.$key)!=null?Cache::get($host.'-'.$key):Cache::put($host.'-'.$key,$this->getAttributeTranslated($key),time()+30*24*3600);
