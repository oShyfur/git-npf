...
class Translator
{
    ...
    public function init()
    {
        ...
        $siteId = DB::table('np_structure_domains')->select('site_id')->where('fqdn', $requestDomain)->first();

        if(!empty($siteId)){
            $siteId = $siteId->site_id;
            $defaultLang = DB::table('np_structure_sites')->select('site_default_lang')->where('id', $siteId)->first();
            if(!empty($defaultLang)){
                $this->defaultLocale = $defaultLang->site_default_lang;
            }else{
